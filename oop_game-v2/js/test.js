...
//enable all keyboard buttons and update to use .key CSS class instead of .chosen & .wrong
    let qwertyKey = document.querySelectorAll('#qwerty button'); //target HTML element 'qwerty'
//I want to iterate over each qwerty Button and enable with .key instead of .chosen or .wrong
    for (let i = 0; i < qwertyKey.length; i++) {
        qwertyKey[i].disabled = false;
       // qwertyKey[i].classList.add('key');
